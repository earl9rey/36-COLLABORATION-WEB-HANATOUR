...
const DestinationModal = () => {
  const [selectedRegionIndex, setSelectedRegionIndex] = useState(0); // 왼쪽 탭
  const [selectedCountry, setSelectedCountry] = useState<string | null>(null); // 가운데 탭
  const [recentSearches, setRecentSearches] = useState<string[]>(['호놀룰루', '베트남', '나트랑', '다낭']);
  ...
  return (
    <div className="border-gray800 flex h-[71.1rem] w-[68.3rem] flex-col items-start border">
      {/* 최근 검색 */}
      <div className="flex w-full flex-col items-start gap-[1.7rem] self-stretch px-[2.6rem] py-[3.1rem]">
        <div className="w-full">
          <div className="border-b-gray300 flex items-center justify-between border-b pb-[1.4rem]">
            ...
            <button onClick={handleClearAll} className="text-gray500 sub6-r-12">
              전체삭제
            </button>
          </div>
          <div className="grid w-full grid-cols-2 gap-x-[2.7rem] gap-y-[1rem]">
            {recentSearches.map((name) => (
              <div key={name} className="flex w-full items-center justify-between py-[1.2rem]">
                <p className="text-gray800 sub3-sb-15">{name}</p>
                <img
                  src={closeIcon}
                  alt="X 아이콘"
        ...
        <div className="border-gray300 flex h-[36rem] border">
            ...
            <div className="grid-auto-rows-[1.8rem] grid grid-cols-2 gap-[2.4rem]">
              {countries.map((country) => (
                <div
                  ...
                  <p className="body5-r-14 text-gray800">{country.name}</p>
                  <img
          ...
          {/* 도시 */}
          <div className="flex w-[17.3rem] flex-col gap-[2.4rem] pt-[1.1rem] pr-[3.5rem] pl-[1.6rem]">
            {cities.map((city) => (
              <div key={city.id} className="flex gap-[0.25rem]">
      ...
      {/* 닫기 버튼 */}
      <div className="bg-gray100 flex h-full w-full justify-end py-[0.8rem] pr-[1.8rem]">
